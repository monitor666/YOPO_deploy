...
| ID | 约束内容 | 原因 |
|----|----------|------|
| C-001 | VINS-Fusion 的 IMU 输入**必须**使用 D435i 自带 IMU | D435i 内部已完成 IMU-相机硬件时间同步 |
| C-002 | VINS-Fusion 的图像输入**必须**使用 D435i 双目红外图像 | 红外图像不受环境光影响，纹理更稳定 |
| C-003 | 所有里程计数据**必须**使用 NWU (北-西-上) 坐标系 | YOPO 训练数据使用 NWU 坐标系 |
| C-004 | 深度图分辨率**必须**保持 16:9 宽高比 | 神经网络输入维度固定 |
| C-005 | YOPO 规划器**必须**同时订阅深度图和里程计 | 缺少任一输入会导致规划器无法工作 |
